...
__fastcall TForm3::TForm3(TComponent* Owner)
	: TForm(Owner)
{
	PageControl1->Pages[0]->Caption = "Дитина, група, працівник";
	PageControl1->Pages[1]->Caption = "Кількість дітей на працівника";
	PageControl1->Pages[2]->Caption = "Інформація по певній групі";
}
//---------------------------------------------------------------------------
...
void __fastcall TForm3::CheckListBox1ClickCheck(TObject *Sender)
{
	...
	ADOQuery3->SQL->Text="SELECT Діти.ПІБ AS [ПІБ дитини], Група.[№ групи], Група.Назва AS [Назва групи], Персонал.ПІБ AS [ПІБ працівника], Персонал.Посада FROM (Діти INNER JOIN (Група INNER JOIN Журнал ON Група.[№ групи] = Журнал.[№ групи]) ON Діти.[№ дитини] = Журнал.[№ дитини]) INNER JOIN Персонал ON Група.[№ групи] = Персонал.[№ групи] WHERE (((Група.Назва) In (p1,p2,p3,p4,p5,p6,p7,p8))) ORDER BY Група.[№ групи];";
